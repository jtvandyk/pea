...
=============================
Uses Claude (via the Anthropic API) to extract structured protest event
fields from unstructured news article text.

The extractor follows the meta-codebook schema developed in this project,
extracting:
  - Event identification (date, location, country)
  - Actor information (who organized/participated)
  - Event type (protest, strike, riot, etc.)
  - Claim/issue (what protesters demanded)
...
# Extraction prompt — designed for accuracy on Global South sources
SYSTEM_PROMPT = """You are an expert coder for a protest event analysis (PEA) dataset, 
specializing in the Global South and non-Western contexts.
...
5. For actor names, use the full organization/group name as given in the article.
...
JSON schema for each event:
{
  ...
  "organizer": "organization or group that called the event",
...
def extract_from_article(
    ...
    # Truncate to avoid token limits while keeping most of article
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if not isinstance(events, list):
                log.warning(f"LLM returned non-list: {type(events)}")
                return []

            # Enrich with metadata not asked of LLM
            for event in events:
            ...
            log.warning(f"Anthropic API error (attempt {attempt+1}): {e}")
            if attempt < max_retries:
                time.sleep(2 ** attempt)
        except Exception as e:
            log.error(f"Unexpected extraction error: {e}")
            break
...
def extract_events(
    ...
    """
    Run LLM extraction across all articles.
    ...
    for i, article in enumerate(articles):
        ...
        log.info(f"[{i+1}/{len(articles)}] Extracting events from: {url}...")

        events = extract_from_article(article, client, model)
    ...
    log.info(
        ...
        f"{processed} articles ({skipped} articles had no events)"
    )
    return all_events
